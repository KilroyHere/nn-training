...
OUT_CSV="${ROOT_DIR}/results/mpi_mp_pip_smoke_metrics.csv"
...
SMOKE_MICROBATCHES="${SMOKE_MICROBATCHES:-4}"
...
echo "MPI Pipeline Model-Parallel smoke launch config:"
...
echo "  train args: epochs=${SMOKE_EPOCHS} batch_size=${SMOKE_BATCH} microbatches=${SMOKE_MICROBATCHES} train_samples=${SMOKE_TRAIN_SAMPLES} val_samples=${SMOKE_VAL_SAMPLES}"
...
echo "MPI Pipeline Model-Parallel smoke run complete: ${OUT_CSV}"
