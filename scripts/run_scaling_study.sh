#!/usr/bin/env bash
# Strong-scaling study: runs flat DP and hierarchical DP across a rank ladder
# at two model sizes, capturing per-epoch CSVs and stdout timing logs.
#
...
for bin in "${FLAT_BIN}" "${HIER_BIN}"; do
    if [[ ! -x "${bin}" ]]; then
...
run_one() {
    ...
    echo "  --> ${label} | nodes=${nodes} tasks/node=${tpn} total=${ntasks} hidden=${hidden}"
    echo "      csv=${out_csv}"
