// Local SGD data-parallel executable entrypoint.
#include <iostream>
#include <mpi.h>
#include <string>
...
int main(int argc, char** argv) {
    ...
    const int rc = nn::run_mpi_dp_local_sgd_training(config, &error_message);
    if (rc != 0 && rank == 0) {
    ...
    if (rank == 0) {
        std::cout << "Training finished (mpi-dp-local-sgd). Metrics written to "
                  << config.output_csv << "\n";
    }
    return 0;
}
