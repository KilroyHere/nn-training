// MPI pipeline model-parallel executable entrypoint.
#include <iostream>
#include <mpi.h>
#include <string>
...
int main(int argc, char** argv) {
    ...
    const int rc = nn::mpi_mp::run_mpi_model_parallel_pipeline(
        config, rank, world_size, MPI_COMM_WORLD, &error_message);
    if (rc != 0 && rank == 0) {
    ...
    if (rank == 0) {
        std::cout << "Training finished (mpi-mp-pip). Metrics written to "
                  << config.output_csv << "\n";
    }
    return 0;
}
