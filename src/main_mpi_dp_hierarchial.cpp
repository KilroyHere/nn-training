// MPI data-parallel hierarchial executable entrypoint.
#include <iostream>
#include <mpi.h>
#include <string>
...
int main(int argc, char** argv) {
    ...
    const int rc = nn::run_mpi_dp_hierarchial_training(config, &error_message);
    if (rc != 0 && rank == 0) {
    ...
    if (rank == 0) {
        std::cout << "Training finished (mpi-dp-hierarchial). Metrics written to " << config.output_csv
                  << "\n";
    }
    return 0;
}
