...
trained with SGD and cross-entropy. The project compares four execution
modes:
\begin{itemize}[nosep]
...
\end{itemize}
Model-parallel MPI is implemented as a separate backend and is evaluated
separately because its decomposition objective differs from DP scaling.
...
are exchanged via point-to-point MPI between adjacent layer partitions.
...
\begin{algorithm}[H]
\caption{Model-parallel epoch (layer-partitioned)}
\label{alg:mp}
\begin{algorithmic}[1]
